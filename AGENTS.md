...
- Resume with a validated frame: add `--reuse-document-frame <document-frame.json>`
- Resume exact successful calls: repeat `--replay-successful-calls <prior-run-dir>` as needed
...
- Treat prior run directories as source-bearing private artifacts; exact replay does not make them safe to publish.
- Keep every deterministic repair in lineage, and never upgrade a repaired candidate into a formal fact.
...
A change is ready only when version metadata, `release-manifest.json`, doctor, unittest, the legacy fixture check, secret/path scans, and GitHub Actions agree. See `AGENT_HANDOFF.md` for the copyable onboarding prompt and `REPRODUCIBILITY.md` for capability tiers.
